...
data_EM <- subset_taxa(dob_permutation, ta2 == "ectomycorrhizal")#done
data_AM <- subset_taxa(dob_permutation, ta2 == "arbuscular_mycorrhizal")
data_soilsap <- subset_taxa(dob_permutation, ta2 == "soil_saprotroph")#done
data_littersap <- subset_taxa(dob_permutation, ta2 == "litter_saprotroph")#done
data_plapat <- subset_taxa(dob_permutation, ta2 == "plant_pathogen")#done
data_woodsap <- subset_taxa(dob_permutation, ta2 == "wood_saprotroph")#done
...
richness_subplot5_dob_EM%>%mutate(plotid=substr(richness_subplot5_dob_EM$ncol,1,3))%>%dplyr::rename(richness=nrow)%>%dplyr::group_by(plotid)%>%dplyr::summarise(mean_value=mean(richness),sd_value = sd(richness))->richness_subplot5_dob_EM
...
richness_subplot10_dob_EM=data.frame(nrow=length(a4),ncol=2)# the mean indiactes the mean value for the cores within the same subplot
for (i in 1:length(a4))
{
  ak=dim(richness[[i]])[1]
...
{
  cat("\r", paste(paste0(rep("*", round(i / 1, 0)), collapse = ""), i, collapse = "")) # informs the processing
  data_sub <- subset_samples(data_EM, subplotID20new==a4[i])
  dim1=dim(sample_data(data_sub))[1]
