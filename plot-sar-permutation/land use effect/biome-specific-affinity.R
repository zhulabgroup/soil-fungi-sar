...
kkm%>%data.frame()%>%
  ...
  rename_all(~paste0(c(biomes,"guild")))

# for different fungal guilds

# to see how many are included
# get the response ration for different guilds

# my function to determine the habitat affinity
my_function_biome_response_ratio=function(data,biomes)
{
  biomes=c("Temperate Broadleaf & Mixed Forests","Temperate Conifer Forests",
           "Temperate Grasslands, Savannas & Shrublands","Tropical & Subtropical Moist Broadleaf Forests")
  
response_ratio=list()
for (j in 1:seq_along(biomes))
{
  #cat("\r", paste(paste0(rep("*", round(j / 1, 0)), collapse = ""), j, collapse = "")) 
  dk=subset_samples(data,LABEL==biomes[j])# select the biome of interest
  # to see which are croplands
  subset_samples(dk,type=="cultivatedCrops")->modified_data
  #soil samples
  dim(sample_data(modified_data)%>%data.frame())[1]->n_crop_sample
...
response_ratio_result=list()
for (k in 1:9)
  cat("\r", paste(paste0(rep("*", round(k / 1, 0)), collapse = ""), k, collapse = ""))
{
response_ratio_result[[k]]=my_function_biome_response_ratio(get(data[k]))
}
  
d=my_function_biome_response_ratio(get(data[1]))
...
#when there is not sites, we can find the nearest site
...
for (i in 1:500)
  ...
  cat("\r", paste(paste0(rep("*", round(i / 1, 0)), collapse = ""), i, collapse = "")) #
  sample_names=sample_names(sub_natural_data)
  sampled_names <- replicate(500,sample(sample_names, n_crop_sample,replace = FALSE))
  sampled_physeq <- prune_samples(sampled_names[i], sub_natural_data)
  richness_natural[i]=estimate_richness(sampled_physeq, measures = "Observed")%>%summarize(mean_A = mean(Observed, na.rm = TRUE))%>%as.numeric()

}

richness_modified=estimate_richness(modified_data, measures = "Observed")%>%summarize(mean_A = mean(Observed, na.rm = TRUE))%>%as.numeric()

# the richness in the croplands



#richness_modified=estimate_richness(natural_data, measures = "Observed")%>%summarize(mean_A = mean(Observed, na.rm = TRUE))%>%as.numeric()





# if we just focused the same site
sample_data(dk)%>%data.frame()%>%filter(type!="cultivatedCrops")%>%count(plotIDM)

# there are plot located in the same site
#we do not consider different plots 




dk%>%filter(type=="cultivatedCrops")
