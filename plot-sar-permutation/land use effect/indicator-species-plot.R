...
subset_samples_list <- lapply(unique(sample_meta[[group_col]]), function(type) {
  samples_in_group <- rownames(sample_meta[sample_meta[[group_col]] == type, ])
...
# for each type, get the species associations
...
patterns_replacements <- c("s.cultivatedCrops" = "crop")


comb_data_biome <- lapply(comb_data_biome, function(x) {
  x <- str_replace_all(x, patterns_replacements)
  return(x)
})

my_list <- lapply(my_list, function(x) gsub("apple", "fruit", x))

my_list <- lapply(comb_data_biome, function(x) {
  x <- gsub("s.cultivatedCrops", "crop", x)
  x <- gsub("s.deciduousForest", "DF", x)
  return(x)
})
...
significant_results%>%filter(index==1)%>%mutate(sp=rownames(.))%>%
  left_join(species_life_style,by="sp")%>%
  ...
  filter(primary_lifestyle%in%c("arbuscular_mycorrhizal",
                                ...
                                "epiphyte",
                                "litter_saprotroph",
                                "mycoparasite","plant_pathogen","soil_saprotroph","wood_saprotroph"))%>%
  count(primary_lifestyle)%>%
  arrange(n)%>%tail(6)
  
  # select the top five species
significant_results%>%filter(index==2)

#for the first biome as an example




significant_results%>%mutate(sp=rownames(.))%>%
  left_join(species_life_style,by="sp")%>%dplyr::select(sp,index,primary_lifestyle)%>%
  filter(primary_lifestyle%in%c("arbuscular_mycorrhizal",
                                "ectomycorrhizal",
                                "epiphyte",
                                "litter_saprotroph",
                                "mycoparasite","plant_pathogen","soil_saprotroph","wood_saprotroph"))%>%
  filter(index==6)%>%count(primary_lifestyle)
...
ggraph(network, layout = "fr") +  # Force-directed layout
  ...
  theme_void() +
  scale_color_manual(values = c("Species" = "gold", "Habitat" = "lightblue")) 

V(network)$color <- ifelse(V(network)$name %in% c("OTU9047",  "OTU9079",  "OTU9100"),"blue", "red")




network_data <- data.frame(
  Species = rownames(significant_results),
  Groups = apply(significant_results[, -ncol(significant_results)], 1, function(x) paste(names(which(x > 0)), collapse = ", "))
)

edges <- network_data %>%
  separate_rows(Groups, sep = ", ") %>%
  dplyr::select(Species, Groups) %>%
  dplyr::rename(Source = Species, Target = Groups)
...
ggraph(graph, layout = "fr") +
  geom_edge_link() +
  geom_node_point(aes(color = name), size = 1) +
