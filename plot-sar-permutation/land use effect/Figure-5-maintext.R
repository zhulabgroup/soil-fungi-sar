...
readRDS(file="data_both_effect_rcp585.rds")

readRDS(file="data_both_effect_rcp245.rds")
...
for (m in 1:9){
  ...
  biome_bar_plot_low[[m]]=ggplot(data=data_set1,aes(fill=group,x=LABEL ,y=100*pro))+
    ...
    scale_fill_manual("",breaks=c("Climate gain.Land gain","Climate loss.Land loss"),labels=c("Gain","Loss"),values=c( "#1E61A5","#E27B60"))+
    ...
    scale_x_discrete(full_name[m],breaks=unique(data_set1$LABEL),
                     labels=c("Temperate forests","Conifer forests","Grasslands","Dry forests"))+
    ylab("% of pixels")+
