...
# an error occured for i=1997

# to get the richness at each 5 x 5 m subplots, and if there were more than two cores we just randomly select one oof them 
...
richness_subplot5_neon=data.frame(nrow=length(a4),ncol=2)# the mean indiactes the mean value for the cores within the same subplot
for (i in 1:length(a4))
  {
  ak=dim(richness[[i]])[1]
...
# get the mean value for each plot
