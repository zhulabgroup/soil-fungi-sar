...
# the impact of cliamte variables on the estimated z values
...
p1=ggplot(data=varp_new_noplant%>%filter(Fractions>0&guild=="all"), aes(x = guild, y = Fractions, fill = type)) +
  ...
  scale_fill_manual(expression(italic(R["Fixed"]^2)*" =56.4%"),breaks=unique(varp_new_noplant$type),
...
plot_grid(p_z_pva,p_z_distribu,p_z_guild,p_effects,ncol=2)
