...
dk=variogram(SOC.bc~1,data = train.xy)
...
dk=variogram(residuals.glm~1,data = train.xy_z,cutoff=35,width=35/15)
plot(dk)
...
dk=variogram(residuals.glm~1,data = train.xy_z)# residuals of the models


v.rf<-variogram(residuals.rf~ 1, data = train.xy_z,cutoff=40, width=40/15)

v.rf<-variogram(residuals.rf~ 1, data = train.xy_z,cutoff=40, width=40/15)
plot(v.rf)


m.rf<-vgm(0.015,"Exp",12,0.02)

m.rf<-vgm(0.05,"Exp",15,0.025)
...
data=obs[,c("lon","lat")]%>%mutate(Grid_Lat = cut(lon, breaks = xgrid), Grid_Lon = cut(lat, breaks = ygrid))%>%mutate(grid=paste(Grid_Lat,"*",Grid_Lon))
...
train.xy_agg=data[,c("zbc","lon","lat")]# the mean of the lon and lat
...
v.glm<-variogram(residuals.glm~ 1, data = train.xy_agg,cutoff=40, width=40/15)

plot(v.glm)

m.glm<-vgm(0.015,"Exp",11,0.033)# based on estimation of the semivariance
...
# the model predicted values for the new locations
grid.xy_agg$GLM_agg <- predict(GLM_agg, grid.df_agg)# the x and y disappear
# the residuals generated by the kriging
...
###
# residuals predicted by the kriging
grid.xy_agg$SK.GLM_agg<-SK.GLM_agg$var1.pred

# Add GLM predicted + SK pedicted residuals
...
p1=ggplot(predz_agg) +
  ...
  scale_color_gradient(expression(Z),low = "blue", high = "yellow",limits=c(0.64, 0.81))+
  theme(legend.position =c(0.2,0.35), 
        legend.key.size = unit(0.15, "inches"),
        guides(color = guide_legend(nrow = 2, byrow = TRUE)),
        legend.title = element_text(size=8),
...
v.rf<-variogram(residuals.rf~ 1, data = train.xy_agg,cutoff=35, width=35/15)
...
set.seed(1856)
models<-caretList(train.x_agg, RESPONSE,
...
train.xy_agg$REG.SOC.bc<-predict(stack.rf,train.x_agg)

train.xy_agg$residuals.stack<-(train.xy_agg$zbc-train.xy_agg$REG.SOC.bc)

v.stack<-variogram(residuals.stack~ 1, data = train.xy_agg,cutoff=35,width=35/15)

v.stack<-variogram(residuals.stack~ 1,data=train.xy_agg)
...
grid.xy_agg$stack <- predict(stack.rf, grid.df_agg)
...
grid.xy_agg$SK.stack<-SK.stack$var1.pred
# Add RF predicted + SK preedicted residuals
grid.xy_agg$RK.stack.bc<-(grid.xy_agg$stack+grid.xy_agg$SK.stack)


##
k1<--2.472824                                 
grid.xy_agg$RK.stack <-((grid.xy_agg$RK.stack.bc *-2.472824+1)^k1)
summary(grid.xy_agg)


attributes_df <- data.frame(grid.xy_agg$RK.stack)
coordinates_df <- data.frame(coordinates(grid.xy_agg))

predz_agg_sta=cbind(coordinates_df,attributes_df)
names(predz_agg_sta)[3]="zvalue"

###

ggplot(predz_agg_sta) +
  geom_point(data=predz_agg_sta,pch=15,aes(x=lon, y=lat,color=zvalue), size=0.275)+
  scale_color_gradient(expression(Z),low = "blue", high = "yellow")+
  theme(legend.position =c(0.2,0.35), 
        legend.key.size = unit(0.15, "inches"),
        guides(color = guide_legend(nrow = 2, byrow = TRUE)),
...
rich=rowSums(no_na_prediction[,3:dim(no_na_prediction)[2]])
rich=data.frame(rich)

grid_rich=cbind(no_na_prediction[,1:2],rich)

p1=ggplot(grid_rich) +
  geom_point(data=grid_rich,pch=15,aes(x=lon, y=lat,color=rich), size=0.275)+
  scale_color_gradient(expression(Z),low = "blue", high = "yellow")+
  theme(legend.position =c(0.2,0.35), 
        legend.key.size = unit(0.15, "inches"),
        guides(color = guide_legend(nrow = 2, byrow = TRUE)),
        ...
        panel.background = element_rect(fill = "NA"), 
        panel.border = element_rect(color = "black", size = 1.5, fill = NA))+
  xlab("Current richness")+
  ylab("")+
  xlim(-175,-42)

## species loss and the z value

predz_agg=mutate(predz_agg,code=rownames(predz_agg))

grid_rich=mutate(grid_rich,code=rownames(grid_rich))

grid_rich_z=merge(predz_agg,grid_rich[,3:4],by="code")

# suggest we will have a 20% habitat loss

Snew/Soriginal=(Anew/Aoriginal)^z

sratio=(0.8)^grid_rich_z$zvalue
snew=sratio*grid_rich_z$rich
sloss=grid_rich_z$rich-snew

dd=cbind(grid_rich_z,snew,sloss)



p2=ggplot(dd) +
  geom_point(data=dd,pch=15,aes(x=lon, y=lat,color=snew), size=0.275)+
  scale_color_gradient(expression(Z),low = "blue", high = "yellow")+
  theme(legend.position =c(0.2,0.35), 
        legend.key.size = unit(0.15, "inches"),
        guides(color = guide_legend(nrow = 2, byrow = TRUE)),
        ...
        panel.background = element_rect(fill = "NA"), 
        panel.border = element_rect(color = "black", size = 1.5, fill = NA))+
  xlab("Richness under 20% habitat loss")+
  ylab("")+
  xlim(-175,-42)

p3=ggplot(dd) +
  geom_point(data=dd,pch=15,aes(x=lon, y=lat,color=sloss), size=0.275)+
  scale_color_gradient(expression(Z),low = "blue", high = "yellow")+
  theme(legend.position =c(0.2,0.35), 
        legend.key.size = unit(0.15, "inches"),
        guides(color = guide_legend(nrow = 2, byrow = TRUE)),
        ...
        panel.border = element_rect(color = "black", size = 1.5, fill = NA))+
  ylab("")+
  xlab(" Species loss under 20% habiat loss")+
  xlim(-175,-42)
