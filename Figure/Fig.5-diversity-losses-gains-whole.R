  ...
  if (i > 1) {
    ...
    for (m in 1:4) {
      pp[[m]] <- ggplot(data = com_data_climate_guild[[i]][[m]], aes(fill = change, y = biome, x = 100 * origin_mean)) +
        ...
        geom_segment(
          data = com_data_climate_guild[[i]][[m]],
        ...
        ylab("") +
        geom_text(
          data = com_data_climate_guild[[i]][[m]] %>% filter(change == "Negative"),
        ...
        geom_text(
          data = com_data_climate_guild[[i]][[m]] %>% filter(change == "Positive"),
          aes(y = biome, x = 7), size = 3.5, vjust = -1.4,
        ...
        scale_y_discrete(
          breaks = unique(com_data_climate_guild[[i]][[m]]$biome), position = "right",
