...
my_function_direction_overall <- function(data) {
  data %>% mutate(type_C = case_when(
    climate_effect < 0 ~ "C-loss",
    climate_effect > 0 ~ "C-gain",
    is.na(climate_effect) ~ "C-NA",
    climate_effect == 0 ~ "C-no"
  )) -> df2
  data %>% mutate(type_L = case_when(
    land_effect < 0 ~ "L-loss",
  ...
  df3 %>%
    mutate(joint_effect = paste(df3$type_L, "_", df3$type_C)) %>%
    mutate(group = case_when(
      joint_effect == "L-loss _ C-loss" ~ "Climate loss.Land loss",
...
common_theme <- theme(
  legend.position = c(0.02611, 0.4660),
...
theme_grid_plot <- theme(
  axis.text = element_text(size = 10, hjust = 0),
...
for (m in 1:9) {
  ...
  for (i in 1:2)
  {
    ...
    data %>%
      left_join(data_percent %>% dplyr::rename(Combination = group), by = "Combination") %>%
      arrange(Var1) %>%
...
select_biome <- c(
  "Temperate Grasslands, Savannas & Shrublands", "Temperate Conifer Forests",
...
for (m in 1:9) {
  ...
  for (i in 1:2)
  {
    ...
    table(grid_level_biomes$LABEL) %>%
      data.frame() %>%
      filter(Var1 %in% select_biome) %>%
      rename_all(~ paste0(c("LABEL", "number"))) -> number_cell_biome

    # to merge the data

    temp_data %>%
      left_join(number_cell_biome, by = "LABEL") %>%
...
for (m in 1:9) {
  ...
  data_set1$LABEL <- factor(data_set1$LABEL, levels = c(
  ...
  biome_bar_plot_low[[m]] <- ggplot(data = data_set1, aes(fill = group, x = LABEL, y = 100 * pro)) +
    ...
    scale_x_discrete(full_name[m],
      breaks = unique(data_set1$LABEL),
...
for (m in 1:9) {
  ...
  data_set2$LABEL <- factor(data_set2$LABEL, levels = c(
    "Temperate Broadleaf & Mixed Forests",
    "Temperate Conifer Forests", "Temperate Grasslands, Savannas & Shrublands",
    "Tropical & Subtropical Dry Broadleaf Forests"
  ))
