...
for(j in 1:15){#did a sensitivity analysis
  ...
  for(i in 1:4){
    ...
    min_size <- min(table(sample_meta[[group_col]]))#get the smaller sample size
...
#if we do not consider the samples size
...
for(i in 1:4){
  ...
  isa_result_broad[[i]] <- multipatt(species_presence, select_type, func = "IndVal", control = how(nperm = 999))
}
