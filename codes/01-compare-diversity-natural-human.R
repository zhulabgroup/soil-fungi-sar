...
sample_data(rare_all_guild_biome) %>%
  ...
  group_by(Site) %>%
  summarise(lon = mean(lon, rm.na = TRUE), lat = mean(lat, rm.na = TRUE)) -> site_coordinates
...
all_land_cover <- sample_data(rare_all_guild_biome) %>%
  data.frame() %>%
  distinct(type) %>%
  mutate(broad_type = case_when(
    type %in% c("evergreenForest", "deciduousForest", "mixedForest") ~ "forest",
...
for (i in 1:45)
{
  ...
  match_land[[i]] <- natural_data_sub %>%
    sample_data() %>%
    data.frame() %>%
    dplyr::select(type, historical_type) %>%
    distinct()
}
...
sample_data(rare_all_guild_biome) %>%
  ...
  distinct() %>%
  left_join(human_dominated_plots, by = "Site") %>%
  filter(!is.na(plotIDM)) %>%
  head(45) -> temp_data
...
biome_group <- list()
for (i in 1:4) {
  human_dominated_plots %>%
    left_join(temp_data %>% dplyr::select(plotIDM, LABEL), by = "plotIDM") %>%
    mutate(code = 1:45) %>%
    group_by(LABEL) %>%
    filter(LABEL == biomes[i]) %>%
    pull(code) -> biome_group[[i]]
}
...
for (m in 1:9)
{
  ...
  for (i in c(1:45))
  {
    ...
    human_dominated_plots %>%
      filter(plotIDM == plotid[i]) %>%
      distinct(historical_type) %>%
      pull(historical_type) -> historical_type0

    # the land cover type to be selected from the adjacent natural plots
    all_land_cover %>%
      filter(broad_type %in% historical_type0) %>%
      pull(type) -> select_type
    unique(human_dominated_plots$Site) -> modified_site

    subset_samples(get(data[m]), type != "cultivatedCrops") -> natural_data


    sample_data(natural_data) %>%
      data.frame() %>%
      distinct(Site) %>%
      pull(Site) -> natural_site
    ...
    natural_data_sub %>%
      sample_data() %>%
      data.frame() %>%
      dplyr::select(type) -> natural_land_use
    modified_data %>%
      sample_data() %>%
      data.frame() %>%
      dplyr::select(type) -> modified_land_use
...
for (m in 1:9)
{
  ...
  for (j in 1:4)
  {
    ...
    for (i in 1:plot_number[j])
    {
      ...
      richness_nature <- out3$iNextEst$size_based %>%
        filter(t == n_sample_crop) %>%
        dplyr::select(qD, qD.LCL, qD.UCL)
...
for (m in 1:9)
{
  do.call(rbind, richness_compare_crop_nature_guild[[m]]) %>%
    data.frame() %>%
    mutate(plotid = unlist(biome_group)) %>%
    mutate(biomes = rep(biome_select, times = plot_number)) %>%
    left_join(temp, by = "plotid") %>%
    rename_all(~ paste0(c("low", "mean_nature", "up", "mean_crop", "plotid", "biome", "site", "plotIDM"))) -> df1

  df1 %>%
    dplyr::select(mean_nature, biome, site, plotIDM) %>%
