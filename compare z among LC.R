...
for (i in 1:length(guild))
     {
       guild_spe=subset(d1,guild==guild[i])
       ...
       anova=oneway.test(z ~ type, data = guild_spe, na.action = na.omit, var.equal = FALSE)
...
# creat the plots
  ...
  p4=ggboxplot(data4, x = "type", y = "z", fill = "type", outlier.shape = NA) +
    ...
    xlab("Epiphyte")+
    ylim(0,4)+
  scale_fill_manual("", breaks = od[[4]]$type, values = c("tan", "mediumseagreen","wheat", "purple","cornsilk", "cadetblue1", "greenyellow", "lavender"), 
                    labels = c( "pastureHay(N=17)", "evergreenForest(N=92)","woodyWetlands(N=29)" ,"mixedForest(N=20)","deciduousForest(N=95)","grassland\nHerbaceous(N=64)","shrubScrub(N=51)","cultivatedCrops(N=22)" )) 
  
# parasitic fungi
  round(data.frame(p.adjust(comp[[5]][,3])),digits = 3)
  
  data5=subset(d1,guild==guild[5])
...
# for acm
  ...
  p6=ggboxplot(data6, x = "type", y = "z", fill = "type", outlier.shape = NA) +
    ...
    geom_hline(yintercept = 0.7559199, linetype = "dashed", color = "red") +
    annotate("text", x = 1, y = 1.25, label = "e", size = 6) +
    annotate("text", x = 2, y = 1.485, label = "d", size = 6) +
    annotate("text", x = 3, y = 1.325403285, label = "cd", size = 6) +
    annotate("text", x = 4, y = 1.4038526, label = "cd", size = 6) +
    annotate("text", x = 5, y = 1.427, label = "bc", size = 6) +
    annotate("text", x = 6, y = 1.630584860525, label = "b", size = 6) +
    annotate("text", x = 7, y = 1.576023, label = "ab", size = 6) +
    annotate("text", x = 8, y = 1.50652, label = "a", size = 6) +
    xlab("ACM")+
    ylim(0,4)+
  scale_fill_manual("", breaks = od[[6]]$type, values = c( "cadetblue1","greenyellow", "lavender", "tan","cornsilk",  "mediumseagreen","wheat","purple"), 
                    labels = c( "grassland\nHerbaceous(N=64)","shrubScrub(N=51)","cultivatedCrops(N=22)","pastureHay(N=17)"  ,"deciduousForest(N=96)" ,"evergreenForest(N=93)","woodyWetlands(N=29)", "mixedForest(N=20)")) 
  ...
  p7=ggboxplot(data7, x = "type", y = "z", fill = "type", outlier.shape = NA) +
    ...
    ylim(0,4)+
  scale_fill_manual("", breaks = od[[7]]$type, values = c("cornsilk","tan", "purple", "cadetblue1","wheat" , "greenyellow", "mediumseagreen"  , "lavender"), 
 ...
 # plant pathogens
  ...
  p8=ggboxplot(data8, x = "type", y = "z", fill = "type", outlier.shape = NA) +
    ...
    geom_hline(yintercept = 0.7559199, linetype = "dashed", color = "red") +
    annotate("text", x = 1, y = 1.25, label = "e", size = 6) +
    annotate("text", x = 2, y = 1.285, label = "de", size = 6) +
    annotate("text", x = 3, y = 1.403285, label = "d", size = 6) +
    annotate("text", x = 4, y = 1.4038526, label = "cd", size = 6) +
    annotate("text", x = 5, y = 1.3627, label = "c", size = 6) +
    annotate("text", x = 6, y = 1.54860525, label = "bc", size = 6) +
    annotate("text", x = 7, y = 1.576023, label = "b", size = 6) +
    annotate("text", x = 8, y = 1.4552, label = "a", size = 6) +
    xlab("Plant pathogens")+
    ylim(0,4)+
  scale_fill_manual("", breaks = od[[8]]$type, values = c("tan", "cadetblue1","greenyellow", "lavender", "cornsilk", "wheat", "mediumseagreen","purple"), 
                    labels = c( "pastureHay(N=17)", "grassland\nHerbaceous(N=64)","shrubScrub(N=51)","cultivatedCrops(N=22)"  ,"deciduousForest(N=96)","woodyWetlands(N=29)" ,"evergreenForest(N=93)", "mixedForest(N=20)")) 
  
  plot_grid(p6,p3,p1,p2,p7,p4,p5,p8,ncol=2,labels=c("(a)","(b)","(c)","(d)","(e)","(f)","(g)","(h)"),label_x = 0.1,label_size = 20)
# 14x25 pdf