# estimate the z values based on functional guilds
library(phyloseq)
library(tidyverse)
library(doParallel)
library(permute)
ft=read.csv("FungalTraits_1.2_ver_16Dec_2020.csv",sep=",",header=T)
ft=ft[,c("GENUS","primary_lifestyle")]
# get the genus name of all the taxa in the full dataset
d=tax_table(rare_all)[,6]
d=data.frame(d)

names(ft)=c("genus","guild")
# assign guild types to each taxa
d=left_join(d,ft,by="genus")
# change it to the tax-table format
d=tax_table(d)
row.names(d)=row.names(tax_table(rare_all))
#adding the guild column to the existing phyloseq object
rare_all=merge_phyloseq(rare_all,d)

### choose a specific functional guild and to see how many plots it occurs


guild=tax_table(rare_all)
guild=data.frame(guild)
guild=unique(guild$ta2)
guild=guild[c(1,3:24,26:29)]# removed two categories of "" and 'NA'

b=vector("list",length(guild))
for(i in 1:length(guild))
  {
  a=subset_taxa(rare_all,ta2==guild[i])# must first select the guild and then check the sample and taxa sums
  a<- subset_samples(a , !is.na(lon) & !is.na(lat))
  a <- subset_taxa(a, taxa_sums(a) > 0)
  a <- subset_samples(a, sample_sums(a) > 0)
  b[[i]]=dim(otu_table(a))
}

c=matrix(nrow=27,ncol=2)
for( i in 1:27)
  {
  c[i,1]=b[[i]][1]
  c[i,2]=b[[i]][2]
}
cbind(guild,c)# look at the number of taxa for each guild and how many cores they occur

#some of the guilds are very rich in taxa, need to determine guild-specific z values
# i would not use Qin's criteria to group guilds as saprotroph on the litter, soil and wood my differ widely in their life strateges

1.# for the "soil_saprotroph"
m=subset_taxa(rare_all,ta2=="soil_saprotroph")# select a guild
m<- subset_samples(m, !is.na(lon) & !is.na(lat))
m<- subset_taxa(m, taxa_sums(m) > 0)
m<- subset_samples(m, sample_sums(m) > 0)


# add a plotID for the data

d=sample_data(m)
table(d$Project)# the first 908 rows are dob sites while the remaining 5470 rows are NEON sites
d1=data.frame(d[,c("geneticSampleID","Site")])
plotID=substr(d1$geneticSampleID,1,8)
d1=cbind(d1,plotID)
iddob=d1$Site[1:table(d$Project)[[1]]]#a site corresponds to a plot
idneon=d1$plotID[(table(d$Project)[[1]]+1):sum(table(d$Project))]# a plotID corresponds to a plot
plotIDM=data.frame(c(iddob,idneon))

names(plotIDM)="plotIDM"# the plotID used for the SAR
row.names(plotIDM)=row.names(sample_data(m))#an important step to merge the data
plotIDM=sample_data(plotIDM)
d<- merge_phyloseq(m, plotIDM)# merge the new plotid with the initial data 

a1= sample_data(d)
a1=unique(a1$plotIDM)

set.seed(10101)
times=30
power.z <- vector("list", length(a1))

for (i in 1:length(a1))
{
  cat('\r',paste(paste0(rep("*", round(i/ 1, 0)), collapse = ''), i, collapse = ''))# informs the processing
  data_sub <- subset_samples(d, plotIDM==a1[i])
  dim1 <- dim(otu_table(data_sub)) # the number of samples in one site
  if (dim1[1] >= 3)# we can construct a linear regression model with at least three cores
  {
    cl <- makeCluster(3)
    registerDoParallel(cl)
    power.z[[i]] <- foreach(i = 1:times, .combine = "cbind", .packages = c("phyloseq")) %dopar%{
      species <- vector(length = dim1[1]) # create a vector to save diversity
      for (j in 1:dim1[1]) 
      { 
        # randomly sample j samples in the plot
        flag <- rep(FALSE, dim1[1])
        flag[sample(1:dim1[1], j)] <- TRUE
        temp <- merge_samples(data_sub, flag, function(x) mean(x, na.rm = TRUE)) # the j samples aggregated by mean
        species[j] <- sum(otu_table(temp)["TRUE"] > 0)
      }
      ex <- as.data.frame(cbind("A"=c(1:dim1[1]),species))
      temp<- summary(lm(log(species)~log(A),ex))[["coefficients"]]# extract the estimated c and z values
      return(c(temp[2,1], temp[1,1]))
    }
    stopCluster(cl)
  }
  else
  {
    power.z[[i]]=matrix(10,nrow=2,ncol = dim1[1])#the number 10 is randomly selected to create a matrix for the plots with < 3 cores to avoid NULL output
  }
}

soilsap_z=matrix(nrow=length(a1),ncol=30)# get the 30 simulated z values for all the plots
for(i in 1:length(a1)){
  soilsap_z[i,]=power.z[[i]][1,]
}

soilsap_z=data.frame(a1,soilsap_z)
write.csv(soilsap_z,"soilsap_z.ranall.csv")

soilsap_c=matrix(nrow=length(a1),ncol=30)# get the 30 simulated c values for the neon site
for(i in 1:length(a1)){
  soilsap_c[i,]=power.z[[i]][2,]
}

soilsap_c=data.frame(a1,soilsap_c)
write.csv(soilsap_c,"soilsap_c.ranall.csv")

2.## for plant pathogen


m=subset_taxa(rare_all,ta2=="plant_pathogen" )# select a guild
m<- subset_samples(m, !is.na(lon) & !is.na(lat))
m<- subset_taxa(m, taxa_sums(m) > 0)
m<- subset_samples(m, sample_sums(m) > 0)


# add a plotID for the data

d=sample_data(m)
table(d$Project)# the first 908 rows are dob sites while the remaining 5470 rows are NEON sites
d1=data.frame(d[,c("geneticSampleID","Site")])
plotID=substr(d1$geneticSampleID,1,8)
d1=cbind(d1,plotID)
iddob=d1$Site[1:table(d$Project)[[1]]]#a site corresponds to a plot
idneon=d1$plotID[(table(d$Project)[[1]]+1):sum(table(d$Project))]# a plotID corresponds to a plot
plotIDM=data.frame(c(iddob,idneon))

names(plotIDM)="plotIDM"# the plotID used for the SAR
row.names(plotIDM)=row.names(sample_data(m))#an important step to merge the data
plotIDM=sample_data(plotIDM)
d<- merge_phyloseq(m, plotIDM)# merge the new plotid with the initial data 

a1= sample_data(d)
a1=unique(a1$plotIDM)

set.seed(10104)
times=30
power.z <- vector("list", length(a1))

for (i in 1:length(a1))
{
  cat('\r',paste(paste0(rep("*", round(i/ 1, 0)), collapse = ''), i, collapse = ''))# informs the processing
  data_sub <- subset_samples(d, plotIDM==a1[i])
  dim1 <- dim(otu_table(data_sub)) # the number of samples in one site
  if (dim1[1] >= 3)# we can construct a linear regression model with at least three cores
  {
    cl <- makeCluster(3)
    registerDoParallel(cl)
    power.z[[i]] <- foreach(i = 1:times, .combine = "cbind", .packages = c("phyloseq")) %dopar%{
      species <- vector(length = dim1[1]) # create a vector to save diversity
      for (j in 1:dim1[1]) 
      { 
        # randomly sample j samples in the plot
        flag <- rep(FALSE, dim1[1])
        flag[sample(1:dim1[1], j)] <- TRUE
        temp <- merge_samples(data_sub, flag, function(x) mean(x, na.rm = TRUE)) # the j samples aggregated by mean
        species[j] <- sum(otu_table(temp)["TRUE"] > 0)
      }
      ex <- as.data.frame(cbind("A"=c(1:dim1[1]),species))
      temp<- summary(lm(log(species)~log(A),ex))[["coefficients"]]# extract the estimated c and z values
      return(c(temp[2,1], temp[1,1]))
    }
    stopCluster(cl)
  }
  else
  {
    power.z[[i]]=matrix(10,nrow=2,ncol = dim1[1])#the number 10 is randomly selected to create a matrix for the plots with < 3 cores to avoid NULL output
  }
}

plantpat_z=matrix(nrow=length(a1),ncol=30)# get the 30 simulated z values for all the plots
for(i in 1:length(a1)){
  plantpat_z[i,]=power.z[[i]][1,]
}

plantpat_z=data.frame(a1,plantpat_z)
write.csv(plantpat_z,"plantpat_z.ranall.csv")

plantpat_c=matrix(nrow=length(a1),ncol=30)# get the 30 simulated c values for the neon site
for(i in 1:length(a1)){
  plantpat_c[i,]=power.z[[i]][2,]
}

plantpat_c=data.frame(a1,plantpat_c)
write.csv(all_c,"plantpat_c.ranall.csv")

3.# for the ECM guild

m=subset_taxa(rare_all,ta2=="ectomycorrhizal" )# select a guild
m<- subset_samples(m, !is.na(lon) & !is.na(lat))
m<- subset_taxa(m, taxa_sums(m) > 0)
m<- subset_samples(m, sample_sums(m) > 0)


# add a plotID for the data

d=sample_data(m)
table(d$Project)# the first 908 rows are dob sites while the remaining 5470 rows are NEON sites
d1=data.frame(d[,c("geneticSampleID","Site")])
plotID=substr(d1$geneticSampleID,1,8)
d1=cbind(d1,plotID)
iddob=d1$Site[1:table(d$Project)[[1]]]#a site corresponds to a plot
idneon=d1$plotID[(table(d$Project)[[1]]+1):sum(table(d$Project))]# a plotID corresponds to a plot
plotIDM=data.frame(c(iddob,idneon))

names(plotIDM)="plotIDM"# the plotID used for the SAR
row.names(plotIDM)=row.names(sample_data(m))#an important step to merge the data
plotIDM=sample_data(plotIDM)
d<- merge_phyloseq(m, plotIDM)# merge the new plotid with the initial data 

a1= sample_data(d)
a1=unique(a1$plotIDM)

set.seed(10101)
times=30
power.z <- vector("list", length(a1))

for (i in 1:length(a1))
{
  cat('\r',paste(paste0(rep("*", round(i/ 1, 0)), collapse = ''), i, collapse = ''))# informs the processing
  data_sub <- subset_samples(d, plotIDM==a1[i])
  dim1 <- dim(otu_table(data_sub)) # the number of samples in one site
  if (dim1[1] >= 3)# we can construct a linear regression model with at least three cores
  {
    cl <- makeCluster(3)
    registerDoParallel(cl)
    power.z[[i]] <- foreach(i = 1:times, .combine = "cbind", .packages = c("phyloseq")) %dopar%{
      species <- vector(length = dim1[1]) # create a vector to save diversity
      for (j in 1:dim1[1]) 
      { 
        # randomly sample j samples in the plot
        flag <- rep(FALSE, dim1[1])
        flag[sample(1:dim1[1], j)] <- TRUE
        temp <- merge_samples(data_sub, flag, function(x) mean(x, na.rm = TRUE)) # the j samples aggregated by mean
        species[j] <- sum(otu_table(temp)["TRUE"] > 0)
      }
      ex <- as.data.frame(cbind("A"=c(1:dim1[1]),species))
      temp<- summary(lm(log(species)~log(A),ex))[["coefficients"]]# extract the estimated c and z values
      return(c(temp[2,1], temp[1,1]))
    }
    stopCluster(cl)
  }
  else
  {
    power.z[[i]]=matrix(10,nrow=2,ncol = dim1[1])#the number 10 is randomly selected to create a matrix for the plots with < 3 cores to avoid NULL output
  }
}

ecm_z=matrix(nrow=length(a1),ncol=30)# get the 30 simulated z values for all the plots
for(i in 1:length(a1)){
  ecm_z[i,]=power.z[[i]][1,]
}

ecm_z=data.frame(a1,ecm_z)
write.csv(ecm_z,"ecm_z.ranall.csv")

ecm_c=matrix(nrow=length(a1),ncol=30)# get the 30 simulated c values for the neon site
for(i in 1:length(a1)){
  ecm_c[i,]=power.z[[i]][2,]
}

ecm_c=data.frame(a1,ecm_c)
write.csv(ecm_c,"ecm_c.ranall.csv")


##
2.#for the arbuscular_mycorrhizal
m=subset_taxa(rare_all,ta2=="arbuscular_mycorrhizal" )# select a guild
m<- subset_samples(m, !is.na(lon) & !is.na(lat))
m<- subset_taxa(m, taxa_sums(m) > 0)
m<- subset_samples(m, sample_sums(m) > 0)


# add a plotID for the data

d=sample_data(m)
table(d$Project)# the first 908 rows are dob sites while the remaining 5470 rows are NEON sites
d1=data.frame(d[,c("geneticSampleID","Site")])
plotID=substr(d1$geneticSampleID,1,8)
d1=cbind(d1,plotID)
iddob=d1$Site[1:table(d$Project)[[1]]]#a site corresponds to a plot
idneon=d1$plotID[(table(d$Project)[[1]]+1):sum(table(d$Project))]# a plotID corresponds to a plot
plotIDM=data.frame(c(iddob,idneon))

names(plotIDM)="plotIDM"# the plotID used for the SAR
row.names(plotIDM)=row.names(sample_data(m))#an important step to merge the data
plotIDM=sample_data(plotIDM)
d<- merge_phyloseq(m, plotIDM)# merge the new plotid with the initial data 

a1= sample_data(d)
a1=unique(a1$plotIDM)

set.seed(10102)
times=30
power.z <- vector("list", length(a1))

for (i in 1:length(a1))
{
  cat('\r',paste(paste0(rep("*", round(i/ 1, 0)), collapse = ''), i, collapse = ''))# informs the processing
  data_sub <- subset_samples(d, plotIDM==a1[i])
  dim1 <- dim(otu_table(data_sub)) # the number of samples in one site
  if (dim1[1] >= 3)# we can construct a linear regression model with at least three cores
  {
    cl <- makeCluster(3)
    registerDoParallel(cl)
    power.z[[i]] <- foreach(i = 1:times, .combine = "cbind", .packages = c("phyloseq")) %dopar%{
      species <- vector(length = dim1[1]) # create a vector to save diversity
      for (j in 1:dim1[1]) 
      { 
        # randomly sample j samples in the plot
        flag <- rep(FALSE, dim1[1])
        flag[sample(1:dim1[1], j)] <- TRUE
        temp <- merge_samples(data_sub, flag, function(x) mean(x, na.rm = TRUE)) # the j samples aggregated by mean
        species[j] <- sum(otu_table(temp)["TRUE"] > 0)
      }
      ex <- as.data.frame(cbind("A"=c(1:dim1[1]),species))
      temp<- summary(lm(log(species)~log(A),ex))[["coefficients"]]# extract the estimated c and z values
      return(c(temp[2,1], temp[1,1]))
    }
    stopCluster(cl)
  }
  else
  {
    power.z[[i]]=matrix(10,nrow=2,ncol = dim1[1])#the number 10 is randomly selected to create a matrix for the plots with < 3 cores to avoid NULL output
  }
}

acm_z=matrix(nrow=length(a1),ncol=30)# get the 30 simulated z values for all the plots
for(i in 1:length(a1)){
  acm_z[i,]=power.z[[i]][1,]
}

acm_z=data.frame(a1,acm_z)
write.csv(acm_z,"acm_z.ranall.csv")

acm_c=matrix(nrow=length(a1),ncol=30)# get the 30 simulated c values for the neon site
for(i in 1:length(a1)){
  acm_c[i,]=power.z[[i]][2,]
}

acm_c=data.frame(a1,acm_c)
write.csv(acm_c,"acm_c.ranall.csv")

5.# for the litter_saprotroph
m=subset_taxa(rare_all,ta2=="litter_saprotroph" )# select a guild
m<- subset_samples(m, !is.na(lon) & !is.na(lat))
m<- subset_taxa(m, taxa_sums(m) > 0)
m<- subset_samples(m, sample_sums(m) > 0)


# add a plotID for the data

d=sample_data(m)
table(d$Project)# the first 908 rows are dob sites while the remaining 5470 rows are NEON sites
d1=data.frame(d[,c("geneticSampleID","Site")])
plotID=substr(d1$geneticSampleID,1,8)
d1=cbind(d1,plotID)
iddob=d1$Site[1:table(d$Project)[[1]]]#a site corresponds to a plot
idneon=d1$plotID[(table(d$Project)[[1]]+1):sum(table(d$Project))]# a plotID corresponds to a plot
plotIDM=data.frame(c(iddob,idneon))

names(plotIDM)="plotIDM"# the plotID used for the SAR
row.names(plotIDM)=row.names(sample_data(m))#an important step to merge the data
plotIDM=sample_data(plotIDM)
d<- merge_phyloseq(m, plotIDM)# merge the new plotid with the initial data 

a1= sample_data(d)
a1=unique(a1$plotIDM)

set.seed(10102)
times=30
power.z <- vector("list", length(a1))

for (i in 1:length(a1))
{
  cat('\r',paste(paste0(rep("*", round(i/ 1, 0)), collapse = ''), i, collapse = ''))# informs the processing
  data_sub <- subset_samples(d, plotIDM==a1[i])
  dim1 <- dim(otu_table(data_sub)) # the number of samples in one site
  if (dim1[1] >= 3)# we can construct a linear regression model with at least three cores
  {
    cl <- makeCluster(3)
    registerDoParallel(cl)
    power.z[[i]] <- foreach(i = 1:times, .combine = "cbind", .packages = c("phyloseq")) %dopar%{
      species <- vector(length = dim1[1]) # create a vector to save diversity
      for (j in 1:dim1[1]) 
      { 
        # randomly sample j samples in the plot
        flag <- rep(FALSE, dim1[1])
        flag[sample(1:dim1[1], j)] <- TRUE
        temp <- merge_samples(data_sub, flag, function(x) mean(x, na.rm = TRUE)) # the j samples aggregated by mean
        species[j] <- sum(otu_table(temp)["TRUE"] > 0)
      }
      ex <- as.data.frame(cbind("A"=c(1:dim1[1]),species))
      temp<- summary(lm(log(species)~log(A),ex))[["coefficients"]]# extract the estimated c and z values
      return(c(temp[2,1], temp[1,1]))
    }
    stopCluster(cl)
  }
  else
  {
    power.z[[i]]=matrix(10,nrow=2,ncol = dim1[1])#the number 10 is randomly selected to create a matrix for the plots with < 3 cores to avoid NULL output
  }
}

litsap_z=matrix(nrow=length(a1),ncol=30)# get the 30 simulated z values for all the plots
for(i in 1:length(a1)){
  litsap_z[i,]=power.z[[i]][1,]
}

litsap_z=data.frame(a1,litsap_z)
write.csv(litsap_z,"litsap_z.ranall.csv")

litsap_c=matrix(nrow=length(a1),ncol=30)# get the 30 simulated c values for the neon site
for(i in 1:length(a1)){
  litsap_c[i,]=power.z[[i]][2,]
}

litsap_c=data.frame(a1,litsap_c)
write.csv(litsap_c,"litsap_c.ranall.csv")
#

5.# for the litter_saprotroph guild
m=subset_taxa(rare_all,ta2=="litter_saprotroph" )# select a guild
m<- subset_samples(m, !is.na(lon) & !is.na(lat))
m<- subset_taxa(m, taxa_sums(m) > 0)
m<- subset_samples(m, sample_sums(m) > 0)


# add a plotID for the data

d=sample_data(m)
table(d$Project)# the first 908 rows are dob sites while the remaining 5470 rows are NEON sites
d1=data.frame(d[,c("geneticSampleID","Site")])
plotID=substr(d1$geneticSampleID,1,8)
d1=cbind(d1,plotID)
iddob=d1$Site[1:table(d$Project)[[1]]]#a site corresponds to a plot
idneon=d1$plotID[(table(d$Project)[[1]]+1):sum(table(d$Project))]# a plotID corresponds to a plot
plotIDM=data.frame(c(iddob,idneon))

names(plotIDM)="plotIDM"# the plotID used for the SAR
row.names(plotIDM)=row.names(sample_data(m))#an important step to merge the data
plotIDM=sample_data(plotIDM)
d<- merge_phyloseq(m, plotIDM)# merge the new plotid with the initial data 

a1= sample_data(d)
a1=unique(a1$plotIDM)

set.seed(10103)
times=30
power.z <- vector("list", length(a1))

for (i in 1:length(a1))
{
  cat('\r',paste(paste0(rep("*", round(i/ 1, 0)), collapse = ''), i, collapse = ''))# informs the processing
  data_sub <- subset_samples(d, plotIDM==a1[i])
  dim1 <- dim(otu_table(data_sub)) # the number of samples in one site
  if (dim1[1] >= 3)# we can construct a linear regression model with at least three cores
  {
    cl <- makeCluster(3)
    registerDoParallel(cl)
    power.z[[i]] <- foreach(i = 1:times, .combine = "cbind", .packages = c("phyloseq")) %dopar%{
      species <- vector(length = dim1[1]) # create a vector to save diversity
      for (j in 1:dim1[1]) 
      { 
        # randomly sample j samples in the plot
        flag <- rep(FALSE, dim1[1])
        flag[sample(1:dim1[1], j)] <- TRUE
        temp <- merge_samples(data_sub, flag, function(x) mean(x, na.rm = TRUE)) # the j samples aggregated by mean
        species[j] <- sum(otu_table(temp)["TRUE"] > 0)
      }
      ex <- as.data.frame(cbind("A"=c(1:dim1[1]),species))
      temp<- summary(lm(log(species)~log(A),ex))[["coefficients"]]# extract the estimated c and z values
      return(c(temp[2,1], temp[1,1]))
    }
    stopCluster(cl)
  }
  else
  {
    power.z[[i]]=matrix(10,nrow=2,ncol = dim1[1])#the number 10 is randomly selected to create a matrix for the plots with < 3 cores to avoid NULL output
  }
}

litsap_z=matrix(nrow=length(a1),ncol=30)# get the 30 simulated z values for all the plots
for(i in 1:length(a1)){
  litsap_z[i,]=power.z[[i]][1,]
}

litsap_z=data.frame(a1,litsap_z)
write.csv(litsap_z,"litsap_z.ranall.csv")

litsap_c=matrix(nrow=length(a1),ncol=30)# get the 30 simulated c values for the neon site
for(i in 1:length(a1)){
  litsap_c[i,]=power.z[[i]][2,]
}

litsap_c=data.frame(a1,litsap_c)
write.csv(litsap_c,"litsap_c.ranall.csv")
#

6.# for the wood_saprotroph
m=subset_taxa(rare_all,ta2=="wood_saprotroph" )# select a guild
m<- subset_samples(m, !is.na(lon) & !is.na(lat))
m<- subset_taxa(m, taxa_sums(m) > 0)
m<- subset_samples(m, sample_sums(m) > 0)


# add a plotID for the data

d=sample_data(m)
table(d$Project)# the first 908 rows are dob sites while the remaining 5470 rows are NEON sites
d1=data.frame(d[,c("geneticSampleID","Site")])
plotID=substr(d1$geneticSampleID,1,8)
d1=cbind(d1,plotID)
iddob=d1$Site[1:table(d$Project)[[1]]]#a site corresponds to a plot
idneon=d1$plotID[(table(d$Project)[[1]]+1):sum(table(d$Project))]# a plotID corresponds to a plot
plotIDM=data.frame(c(iddob,idneon))

names(plotIDM)="plotIDM"# the plotID used for the SAR
row.names(plotIDM)=row.names(sample_data(m))#an important step to merge the data
plotIDM=sample_data(plotIDM)
d<- merge_phyloseq(m, plotIDM)# merge the new plotid with the initial data 

a1= sample_data(d)
a1=unique(a1$plotIDM)

set.seed(10102)
times=30
power.z <- vector("list", length(a1))

for (i in 1:length(a1))
{
  cat('\r',paste(paste0(rep("*", round(i/ 1, 0)), collapse = ''), i, collapse = ''))# informs the processing
  data_sub <- subset_samples(d, plotIDM==a1[i])
  dim1 <- dim(otu_table(data_sub)) # the number of samples in one site
  if (dim1[1] >= 3)# we can construct a linear regression model with at least three cores
  {
    cl <- makeCluster(3)
    registerDoParallel(cl)
    power.z[[i]] <- foreach(i = 1:times, .combine = "cbind", .packages = c("phyloseq")) %dopar%{
      species <- vector(length = dim1[1]) # create a vector to save diversity
      for (j in 1:dim1[1]) 
      { 
        # randomly sample j samples in the plot
        flag <- rep(FALSE, dim1[1])
        flag[sample(1:dim1[1], j)] <- TRUE
        temp <- merge_samples(data_sub, flag, function(x) mean(x, na.rm = TRUE)) # the j samples aggregated by mean
        species[j] <- sum(otu_table(temp)["TRUE"] > 0)
      }
      ex <- as.data.frame(cbind("A"=c(1:dim1[1]),species))
      temp<- summary(lm(log(species)~log(A),ex))[["coefficients"]]# extract the estimated c and z values
      return(c(temp[2,1], temp[1,1]))
    }
    stopCluster(cl)
  }
  else
  {
    power.z[[i]]=matrix(10,nrow=2,ncol = dim1[1])#the number 10 is randomly selected to create a matrix for the plots with < 3 cores to avoid NULL output
  }
}

woosap_z=matrix(nrow=length(a1),ncol=30)# get the 30 simulated z values for all the plots
for(i in 1:length(a1)){
  woosap_z[i,]=power.z[[i]][1,]
}

woosap_z=data.frame(a1,woosap_z)
write.csv(woosap_z,"woosap_z.ranall.csv")

woosap_c=matrix(nrow=length(a1),ncol=30)# get the 30 simulated c values for the neon site
for(i in 1:length(a1)){
  woosap_c[i,]=power.z[[i]][2,]
}

woosap_c=data.frame(a1,woosap_c)
write.csv(woosap_c,"woosap_c.ranall.csv")
#
5. # for the parasite, which includes several guilds

pa=data.frame(c("animal_parasite","lichen_parasite","protistan_parasite","algal_parasite" ))
names(pa)="pa"
5.
m=subset_taxa(rare_all,ta2%in%pa$pa)# select a guild
m<- subset_samples(m, !is.na(lon) & !is.na(lat))
m<- subset_taxa(m, taxa_sums(m) > 0)
m<- subset_samples(m, sample_sums(m) > 0)


# add a plotID for the data

d=sample_data(m)
table(d$Project)# the first 908 rows are dob sites while the remaining 5470 rows are NEON sites
d1=data.frame(d[,c("geneticSampleID","Site")])
plotID=substr(d1$geneticSampleID,1,8)
d1=cbind(d1,plotID)
iddob=d1$Site[1:table(d$Project)[[1]]]#a site corresponds to a plot
idneon=d1$plotID[(table(d$Project)[[1]]+1):sum(table(d$Project))]# a plotID corresponds to a plot
plotIDM=data.frame(c(iddob,idneon))

names(plotIDM)="plotIDM"# the plotID used for the SAR
row.names(plotIDM)=row.names(sample_data(m))#an important step to merge the data
plotIDM=sample_data(plotIDM)
d<- merge_phyloseq(m, plotIDM)# merge the new plotid with the initial data 

a1= sample_data(d)
a1=unique(a1$plotIDM)

set.seed(10102)
times=30
power.z <- vector("list", length(a1))

for (i in 1:length(a1))
{
  cat('\r',paste(paste0(rep("*", round(i/ 1, 0)), collapse = ''), i, collapse = ''))# informs the processing
  data_sub <- subset_samples(d, plotIDM==a1[i])
  dim1 <- dim(otu_table(data_sub)) # the number of samples in one site
  if (dim1[1] >= 3)# we can construct a linear regression model with at least three cores
  {
    cl <- makeCluster(3)
    registerDoParallel(cl)
    power.z[[i]] <- foreach(i = 1:times, .combine = "cbind", .packages = c("phyloseq")) %dopar%{
      species <- vector(length = dim1[1]) # create a vector to save diversity
      for (j in 1:dim1[1]) 
      { 
        # randomly sample j samples in the plot
        flag <- rep(FALSE, dim1[1])
        flag[sample(1:dim1[1], j)] <- TRUE
        temp <- merge_samples(data_sub, flag, function(x) mean(x, na.rm = TRUE)) # the j samples aggregated by mean
        species[j] <- sum(otu_table(temp)["TRUE"] > 0)
      }
      ex <- as.data.frame(cbind("A"=c(1:dim1[1]),species))
      temp<- summary(lm(log(species)~log(A),ex))[["coefficients"]]# extract the estimated c and z values
      return(c(temp[2,1], temp[1,1]))
    }
    stopCluster(cl)
  }
  else
  {
    power.z[[i]]=matrix(10,nrow=2,ncol = dim1[1])#the number 10 is randomly selected to create a matrix for the plots with < 3 cores to avoid NULL output
  }
}

para_z=matrix(nrow=length(a1),ncol=30)# get the 30 simulated z values for all the plots
for(i in 1:length(a1)){
  para_z[i,]=power.z[[i]][1,]
}

para_z=data.frame(a1,para_z)
write.csv(para_z,"para_z.ranall.csv")

para_c=matrix(nrow=length(a1),ncol=30)# get the 30 simulated c values for the neon site
for(i in 1:length(a1)){
  para_c[i,]=power.z[[i]][2,]
}

para_c=data.frame(a1,para_c)
write.csv(para_c,"para_c.ranall.csv")
#
8.# for the epiphyte
m=subset_taxa(rare_all,ta2=="epiphyte" )# select a guild
m<- subset_samples(m, !is.na(lon) & !is.na(lat))
m<- subset_taxa(m, taxa_sums(m) > 0)
m<- subset_samples(m, sample_sums(m) > 0)


# add a plotID for the data

d=sample_data(m)
table(d$Project)# the first 908 rows are dob sites while the remaining 5470 rows are NEON sites
d1=data.frame(d[,c("geneticSampleID","Site")])
plotID=substr(d1$geneticSampleID,1,8)
d1=cbind(d1,plotID)
iddob=d1$Site[1:table(d$Project)[[1]]]#a site corresponds to a plot
idneon=d1$plotID[(table(d$Project)[[1]]+1):sum(table(d$Project))]# a plotID corresponds to a plot
plotIDM=data.frame(c(iddob,idneon))

names(plotIDM)="plotIDM"# the plotID used for the SAR
row.names(plotIDM)=row.names(sample_data(m))#an important step to merge the data
plotIDM=sample_data(plotIDM)
d<- merge_phyloseq(m, plotIDM)# merge the new plotid with the initial data 

a1= sample_data(d)
a1=unique(a1$plotIDM)

set.seed(10109)
times=30
power.z <- vector("list", length(a1))

for (i in 1:length(a1))
{
  cat('\r',paste(paste0(rep("*", round(i/ 1, 0)), collapse = ''), i, collapse = ''))# informs the processing
  data_sub <- subset_samples(d, plotIDM==a1[i])
  dim1 <- dim(otu_table(data_sub)) # the number of samples in one site
  if (dim1[1] >= 3)# we can construct a linear regression model with at least three cores
  {
    cl <- makeCluster(3)
    registerDoParallel(cl)
    power.z[[i]] <- foreach(i = 1:times, .combine = "cbind", .packages = c("phyloseq")) %dopar%{
      species <- vector(length = dim1[1]) # create a vector to save diversity
      for (j in 1:dim1[1]) 
      { 
        # randomly sample j samples in the plot
        flag <- rep(FALSE, dim1[1])
        flag[sample(1:dim1[1], j)] <- TRUE
        temp <- merge_samples(data_sub, flag, function(x) mean(x, na.rm = TRUE)) # the j samples aggregated by mean
        species[j] <- sum(otu_table(temp)["TRUE"] > 0)
      }
      ex <- as.data.frame(cbind("A"=c(1:dim1[1]),species))
      temp<- summary(lm(log(species)~log(A),ex))[["coefficients"]]# extract the estimated c and z values
      return(c(temp[2,1], temp[1,1]))
    }
    stopCluster(cl)
  }
  else
  {
    power.z[[i]]=matrix(10,nrow=2,ncol = dim1[1])#the number 10 is randomly selected to create a matrix for the plots with < 3 cores to avoid NULL output
  }
}

epiphy_z=matrix(nrow=length(a1),ncol=30)# get the 30 simulated z values for all the plots
for(i in 1:length(a1)){
  epiphy_z[i,]=power.z[[i]][1,]
}

epiphy_z=data.frame(a1,epiphy_z)
write.csv(epiphy_z,"epiphy_z.ranall.csv")

epiphy_c=matrix(nrow=length(a1),ncol=30)# get the 30 simulated c values for the neon site
for(i in 1:length(a1)){
  epiphy_c[i,]=power.z[[i]][2,]
}

epiphy_c=data.frame(a1,epiphy_c)
write.csv(epiphy_c,"epiphy_c.ranall.csv")

