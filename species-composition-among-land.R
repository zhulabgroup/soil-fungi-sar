...
for (i in 1:45)
{
  compare[[i]]=list(match_land[[i]],df4$historical_type[i])
}
...
## to see the beta diversity among plots
...
for (i in 1:45){
  ...
  beta_jaccard <- vegdist(d, method = "bray")# this quantifys the dissimilarity
...
for (i in c(1:45))
{
  ...
  df4%>%filter(plotIDM==plotid[i])%>%distinct(historical_type)%>%pull(historical_type)->historical_type0
