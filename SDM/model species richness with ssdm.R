...
occ=otu_table(agg_data)%>%data.frame()%>%bind_cols(env[,1:2])
...
for (i in 1:8597){
  sdm[[i]] <- modelling('RF', subset(occurrence_data,variable==sp[i]),
                   r_present_northam, Xcol = 'lon', Ycol = 'lat',Pcol="value", Spcol="variable",verbose = FALSE)
...
richness_present=cbind(coords_present,d)%>%data.frame()%>%rename(lon=x,lat=y,richness=d)
...
#future_data=raster::getData(name = 'CMIP5', var = 'bio', res = 10,
...
for (i in 1:8597){
  sdm[[i]] <- modelling('RF', subset(occurrence_data,variable==sp[i]),
                        r_future_northam, Xcol = 'lon', Ycol = 'lat',Pcol="value", Spcol="variable",verbose = FALSE)
...
data=data.frame(data)
d=rowSums(data)# the total richness for each grid cell
richness_future=cbind(coords_present,d)%>%data.frame()%>%rename(lon=x,lat=y,richness=d)
...
PFT_2020=cbind(coords_present,PFT_2020)%>%data.frame()%>%
...
PFT_2100=cbind(coords_present,PFT_2100)%>%data.frame()%>%
...
land_use_change=land_use_change%>%rename(lat=lon,lon=lat)
...
kk=richness_present%>%mutate(future_rich=land_use_change$nocrop^0.3877299*richness)%>%bind_cols(land_use_change[,c("nocrop")])%>%rename_all(~paste0(c("lon","lat","richness","future_rich","land_change_ratio")))%>%mutate(species_change=future_rich-richness,rich_change_rate=species_change/richness)

land_use_change_rich=kk

save(land_use_change_rich,file="land_use_change_rich.RData")
...
#1. cells don't have land-use data (NA) but we have richness data
#2. cells don't have land-use data for both time points(0-0) but we have richness data
...
for(i in 1:119607)
{
  ...
  else if(is.infinite(df[i,3])&&!is.nan(df[i,1]))
  {
    ...
    dff[i,3]=NA
    dff[i,4]=NA
    dff[i,5]=NA# we do not evaluate such changes but richness shoud increase
  }
...
dff%>%data.frame()%>%rename_all(~paste0(c(colnames(df))))%>%bind_cols(land_use_change_rich_nona[,1:2])->dff

land_use_change_rich_nona_conver3=dff#(conver na to the intial richness)




#(9). make some conversions based on different land-change scenarios

land_use_change_rich_nona=subset(land_use_change_rich,richness>0)

df=land_use_change_rich_nona[,(3:7)]
dff=matrix(ncol=5,nrow=119607)
for(i in 1:119607)
{
  cat("\r", paste(paste0(rep("*", round(i / 1, 0)), collapse = ""), i, collapse = "")) # informs the processing
  if(is.nan(df[i,3]))
  {
    dff[i,1]=df[i,1]
    dff[i,2]=df[i,1]# land-use type value in the cell is 0
    dff[i,3]=NA
    dff[i,4]=0
    dff[i,5]=0
  }
  
  else if(is.na(df[i,3]))
  {
    dff[i,1]=df[i,1]
    dff[i,2]=df[i,1]# land-use type value in the cell is 0
    dff[i,3]=NA
  ...
  else if(is.infinite(df[i,3]))
  {
    dff[i,1]=df[i,1]
    dff[i,2]=df[i,1]# land-use type value in the cell is 0
    dff[i,3]=NA
    dff[i,4]=NA
    dff[i,5]=NA# we do not evaluate such changes or it is TBD
  }
...
land_use_change_rich_nona_conver%>%bind_cols(land_use_change_rich_nona[,1:2])->land_use_change_rich_nona_conver

# just look at cell with increased richness
land_use_change_rich_nona_conver%>%mutate(rich_change_rate = ifelse(rich_change_rate > 0, 0, rich_change_rate))->land_use_change_rich_nona_conver1

# just look at cells with decreased richness
land_use_change_rich_nona_conver%>%mutate(rich_change_rate = ifelse(rich_change_rate < 0, 0, rich_change_rate))->land_use_change_rich_nona_conver2

dff%>%mutate(rich_change_rate = ifelse(rich_change_rate > 0, 0, rich_change_rate))->dff_cover1

dff%>%mutate(rich_change_rate = ifelse(rich_change_rate < 0, 0, rich_change_rate))->dff_cover2
...
ggplot(species_change_climate1) +
  ...
  scale_color_gradient(expression("Species change rate %"),low = "purple",high = "gray", na.value = "white",limits=c(-100,0))+
...
p3=ggplot(species_change_climate2) +
  geom_point(data=species_change_climate2,pch=15,aes(x=lon, y=lat,color=rate*100), size=0.275)+
  scale_color_gradient(expression("Species change rate %"),low = "gray",high = "seagreen1", na.value = "white")+
  theme(legend.position =c(0.2,0.35), 
        legend.key.size = unit(0.15, "inches"),
        guides(color = guide_legend(nrow = 2, byrow = TRUE)),
        legend.title = element_text(size=8),
...
p2=ggplot(land_use_change_rich_nona_conver1) +
  geom_point(data=land_use_change_rich_nona_conver1,pch=15,aes(x=lon, y=lat,color=rich_change_rate*100), size=0.275)+
  scale_color_gradient(expression("Species change rate %"),low = "purple",high = "gray",  na.value = "blue")+
  theme(legend.position =c(0.2,0.35), 
        legend.key.size = unit(0.15, "inches"),
        guides(color = guide_legend(nrow = 2, byrow = TRUE)),
        legend.title = element_text(size=8),
...
p4=ggplot(land_use_change_rich_nona_conver2) +
  geom_point(data=land_use_change_rich_nona_conver2,pch=15,aes(x=lon, y=lat,color=rich_change_rate*100), size=0.275)+
  scale_color_gradient(expression("Species change rate %"),low = "gray",high = "seagreen1",  na.value = "blue")+
  theme(legend.position =c(0.2,0.35), 
        legend.key.size = unit(0.15, "inches"),
        guides(color = guide_legend(nrow = 2, byrow = TRUE)),
        legend.title = element_text(size=8),
...
ggplot(land_use_change_rich_nona_conver) +
  geom_point(data=land_use_change_rich_nona_conver,aes(x=lon, y=lat,color=rich_change_rate), size=0.275)+
  scale_color_gradient(expression("Species change rate %"),low = "red",high = "seagreen1",  na.value = "white")+
  theme(legend.position =c(0.2,0.35), 
        legend.key.size = unit(0.15, "inches"),
        guides(color = guide_legend(nrow = 2, byrow = TRUE)),
        legend.title = element_text(size=8),
...
p3=ggplot(land_use_change_rich_nona_conver3) +
  ...
  xlab("Predicted richness in 2100")+
  ylab("")+
  ggtitle("Land-use-change induced")
...
names(land_loss_degree)[8]="land_degree"

climate_loss_degree%>%mutate(location=paste(lon,"*",lat))->climate_loss_degree

land_climate_loss_degree=merge(climate_loss_degree,land_loss_degree[,c(1:5,8:9)],by="location")%>%mutate(com_degree=paste(new_column,"*",land_degree))

save(land_climate_loss_degree,file="land_climate_loss_degree.RData")

ggplot(land_climate_loss_degree) +
  geom_point(data=land_climate_loss_degree,pch=15,aes(x=lon, y=lat,color=com_degree),size=0.55)+

  scale_color_manual(expression("Species change rate %"),breaks=unique(land_climate_loss_degree$com_degree),
                     labels=c("Land use high (22.3%)","Both high (3.7%)","Climate high (12.2%)","Both low (59.0%)","Uncertainty","Uncertainty","Uncertainty","Uncertainty"),
                     values=c("deepskyblue3","black","chocolate1","gray","gray","gray","deepskyblue3","chocolate1")) +
  theme(legend.position =c(0.2,0.35), 
        legend.key.size = unit(0.15, "inches"),
        guides(color = guide_legend(nrow = 2, byrow = TRUE)),
