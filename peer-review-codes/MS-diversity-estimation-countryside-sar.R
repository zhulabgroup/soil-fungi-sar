...
r <- rast(ext(biomes),
  resolution = res(coarser_raster), # the resolution of your targeted raster
  crs = "EPSG:4326"
)
...
my_function_raster <- function(data) {
  ...
  for (i in 1:33) {
    temp <- rast(
      nrows = nrow(cropped_raster), ncols = ncol(cropped_raster),
      xmin = -169.95, xmax = -55.05,
      ymin = 18, ymax = 72,
      crs = crs(cropped_raster)
    ) # change the ranges for the raster
  ...
  df_temp1 <- df_temp1 %>% mutate(
    tree = PFT1 + PFT2 + PFT3 + PFT4 + PFT5 + PFT6 + PFT7 + PFT8, shrup = PFT9 + PFT10 + PFT11,
  ...
  df_temp1 %>%
    bind_cols(No_crop) %>%
    mutate(area = 11637.87^2) -> df_temp1 # total area
  ...
  for (i in 1:9)
  {
    richness_temp <- df_temp1 %>%
      dplyr::select(crop, No_crop, LABEL, area) %>%
      ...
      mutate(richness = mean_cvalue * (affinity * crop / 100 * area + 1 * No_crop / 100 * area)^mean_zvalue) %>%
      as.matrix()
    richness_derived[, i] <- richness_temp[, "richness"] #
  }
  richness_derived <- richness_derived %>%
    data.frame() %>%
