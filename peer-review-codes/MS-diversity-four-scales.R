...
for (m in 1:9)
{
  ...
  cbind(richness_plot_neon,my_list[[m]])%>%data.frame()%>%
...
cbind(plotid=a4,mean_value=richness_mean_subplot40_neon[,1],
                                        sd_value=richness_mean_subplot40_neon[,2],
                                        rep(1600,length.out=n))%>%data.frame()%>%mutate(guild=rep(my_list[[m]],n()))
...
tax_table(dob_permutation)%>%data.frame()%>%pull(guild)%>%unique()->all
...
richness_subplot10_dob%>%mutate(plotid=substr(richness_subplot10_dob$ncol,1,3))%>%
...
for (m in 1:9)
{
  ...
  richness_mean_subplot20_dob%>%data.frame()%>%bind_cols(a4)%>%mutate(plotid=substr(a4,1,3))%>%
    group_by(plotid)%>%summarise(mean_value=mean(X1,na.rm=TRUE),sd_value=sd(X1,na.rm=TRUE))%>%
    mutate(area=rep(400,n()),guild=rep(my_list[[m]],n()))->data[[m]]
...
guild_select=c("ectomycorrhizal","arbuscular_mycorrhizal","soil_saprotroph","litter_saprotroph","plant_pathogen","wood_saprotroph",
               "para","epiphyte")
tax_table(dob_permutation)%>%data.frame()%>%pull(ta2)%>%unique()->all
my_list <- c(guild_select[1:8], list(all))
...
cbind(richness_plot_dob,my_list[[m]])%>%data.frame()%>%
  mutate(area=rep(900,n()))%>%
    select(X1,X2,X3,area,X4)%>%
...
richness_mean_subplot40_dob%>%data.frame()%>%mutate(plotid=a4,area=rep(1600,n()),guild=rep(my_list[[m]],n()))%>%
